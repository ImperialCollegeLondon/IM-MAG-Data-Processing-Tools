...
def get_met_from_shcourse(seconds_since_epoch: float) -> datetime:
...
def humanise_timedelta(
    ...
    # credit to https://stackoverflow.com/questions/538666/format-timedelta-to-string
    ...
    full_text = f.format(fmt, **values)

    # regex to remove "00d" etc
    if strip_zeros:
        full_text = re.sub(r"^(\s?0+[wdmhy])+\s+", "", full_text)
